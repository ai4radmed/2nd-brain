...
> 라벨 **`1 첨부저장`** 메일의 첨부를 저장 디렉토리에 받은 뒤, 성공한 메일만 라벨을 **`4 완료`** 로 교체.
...
- **fail-safe** → 첨부 저장이 성공한 메일만 라벨을 교체(실패 시 라벨 유지 → 유실 방지).
...
> **라벨은 고정 규칙입니다 (코드 수정 대상 아님).** 이 스킬은 트리거 `1 첨부저장` → 완료 `4 완료` 를 쓴다 — Gmail 에 **이 두 라벨을 그대로 만들어** 사용하세요(없으면 봇에게 "gog 로 라벨 만들어줘"). 라벨명에 공백이 있어 검색 쿼리는 `label:"1 첨부저장"` 형태를 쓴다.
...
**봇(텔레그램 등) 실사용**: 위 "계정·env 설정 위치"로 컨테이너에 env 를 넣었으면, SKILL.md 의 자연어 트리거("1 첨부저장 처리해줘" / "먼저 몇 건인지 봐줘")로 부르면 된다 — 인라인 env 불필요.

## 알려진 한계

- 첨부 저장 후 **라벨 교체가 실패**하면, 다음 실행 때 같은 첨부가 다시 저장된다(파일명은 `_1`, `_2` 로 충돌 회피 — 덮어쓰지 않음). 완전 멱등이 필요하면 저장 전 처리이력 체크를 추가한다.
- 라우트가 **1개**다. 같은 동사(첨부 저장)로 라벨만 늘리려면 라우트 상수를 늘리고, *다른 동사*(일정 등록·할일 등록 등)가 필요하면 `process_thread` 패턴을 본떠 핸들러를 추가한다.
