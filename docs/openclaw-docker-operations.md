...
| 유형 | 예 | 무인 거처 | 절 |
|---|---|---|---|
| **텍스트형**(gog API·LLM, 브라우저 X) | `gmail-label-actions`(라벨→GWS 후속작업·캡처) | **게이트웨이 cron** (컨테이너 안) | §9a |
| **브라우저형**(playwright headed) | `webmail-watch`(anti-headless SPA), `society-watch` | **headed-Xvfb 사이드카** + host 스케줄러 | §9b |
| **호스트 드레인**(파싱·정제·노트화) | `parser-drain`(extract), `brain-drain`(refine+brainify) | **host systemd-user 타이머** + `claude -p` | §9c |

> 흐름: 게이트웨이 cron 이 캡처(§9a) → 호스트 드레인이 파싱·정제·brainify(§9c). 다중기기에선 **§9c 의 단일 발화 머신 원칙**을 게이트웨이 cron 과 한 단위로 적용.
...
### 9c. 호스트 드레인 타이머 — parser-drain · brain-drain (extract→refine→brainify 무인)

게이트웨이가 메일을 `sources/00_inbox/` 에 떨구면(§9a), 그 뒤 **파싱·정제·노트화**는 게이트웨이 밖 **호스트 systemd-user 타이머** 두 개가 이어받는다. LLM 판단(refine/brainify)을 게이트웨이 agent-turn 이 아니라 호스트 `claude -p` 로 돌려 watchdog·동시성에서 분리한다.

```
[수동] Gmail 라벨 1~8
  └ gmail-label-actions cron(§9a)  → sources/00_inbox/ 캡처
  └ parser-drain.timer (~10분)     → extract: 듀얼 파싱 _parse/{docling,mineru,diff}.json (결정형, claude 0)
  └ brain-drain.timer  (~10분)     → refine(diverge 만 claude -p) + brainify(claude -p) → PARA 동반 노트
```

**설치** (자산은 이 repo 에 포함 — 외부 사용자도 동일 명령):

```bash
# 1) extract 드레인 (듀얼 파서 컨테이너 호출)
cd ~/projects/2nd-brain/docker     && make install-parser-drain
# 2) refine+brainify 드레인 (호스트 claude -p — Claude Code CLI 설치·인증 필요)
cd ~/projects/2nd-brain/automation && make install-brain-drain
# 3) 부팅 후에도 user 타이머 지속 (1회, sudo)
sudo loginctl enable-linger "$USER"
```

각 타이머는 `OnActiveSec`(활성 시 시드) + `OnUnitInactiveSec`(드레인 종료 후 재발화) 패턴이라 긴 드레인과 겹치지 않고, `enable --now`/재로드마다 재무장된다. (⚠️ unit 파일에 **인라인 주석 금지** — systemd 가 그 줄을 bad-setting 으로 조용히 폐기해 재무장이 깨진다. 주석은 별도 줄로.)

**비용·정책**: brain-drain 은 항목당 `claude -p` 를 부른다(refine diverge + brainify). 항목당 `--max-budget-usd` + 드레인당 누적 상한으로 가드(기본 refine $0.50·brainify $0.75·드레인 $5). brainify 는 [자동 우선·주간 감사 정책](./brain-system/automation-weekly-audit.md)을 따라 무인 낙관 배치 + `para_review: pending` 플래그 → 주 1회 감사로 교정.

#### ★ 단일 발화 머신 원칙 (다중기기 필수)

vault 가 여러 PC 에 동기(SyncThing)될 때, **자동발화는 한 번에 한 PC 에서만** 돌려야 한다. 둘 이상이 동시에 켜지면:

- parser-drain 이중 파싱 → 같은 `_parse/` 를 양쪽이 써서 SyncThing 충돌 파일
- brain-drain 중복 노트·claude 비용 2배·원본 이동 경합
- gmail cron 중복 캡처·Telegram 중복 알림

→ 게이트웨이 밖 호스트 발화도 캡처용 게이트웨이 cron 과 **같은 토글 단위**로 묶어, 발화 머신에서만 켠다. 외부 사용자용 plain systemctl on/off/status:

```bash
# 이 PC 의 모든 호스트 발화 타이머
TIMERS="parser-drain.timer brain-drain.timer openclaw-webmail-sidecar.timer"

# status
systemctl --user list-timers $TIMERS --all --no-pager
# on  (발화 머신으로 지정)
...
게이트웨이 cron(§9a) 토글까지 함께 묶으려면 `docker exec <gw> node /app/dist/index.js cron <enable|disable> <id> --token <tok>` 를 병행한다. (저자는 이 둘을 한 번에 토글하는 Claude Code 슬래시 명령 `/cron on|off|status` 로 운용 — 외부 사용자는 위 systemctl + cron CLI 를 직접.)

비발화 머신에서도 **수동 on-demand** 는 언제나 가능: `systemctl --user start parser-drain.service` / `brain-drain.service` (1회 드레인).
