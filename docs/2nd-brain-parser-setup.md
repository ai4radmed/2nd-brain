# 2nd-brain-parser — 문서 파서 컨테이너 (PDF·Office·한글·레거시 → markdown)
...
**파이프라인 위치**: `capture(라벨 라우터) → 00_inbox(바이너리+_thread.md) → **2nd-brain-parser 파싱(여기)** → brainify(LLM 노트)`. brainify(Claude Code)가 이 컨테이너를 서브프로세스로 호출한다.
...
```bash
docker pull ghcr.io/benkorea/2nd-brain-parser:latest
```
...
C="docker compose -f docker/2nd-brain-parser/compose.2nd-brain-parser.yml run --rm 2nd-brain-parser"
$C 2nd-brain-parser parse-docling <PDF경로>
$C 2nd-brain-parser parse-docling <HWP경로>          # hwp/hwpx → LibreOffice 로 docx 자동변환 후 Docling
$C 2nd-brain-parser parse-mineru  <PDF경로>          # PDF 듀얼 체크
$C 2nd-brain-parser diff <docling.md> <mineru.md>
```
...
> `2nd-brain-parser` 가 두 번 나오는 건 정상 — 앞은 compose **서비스명**, 뒤는 **CLI 명령**이다 (`docker compose run <service> <command>`).
...
- **GPU**(수식 PDF) — NVIDIA 호스트에서 오버레이 추가:
  ```bash
  docker compose -f docker/2nd-brain-parser/compose.2nd-brain-parser.yml \
                 -f docker/2nd-brain-parser/compose.2nd-brain-parser.gpu.yml \
                 run --rm 2nd-brain-parser 2nd-brain-parser parse-mineru <pdf>
  ```
...
>            ln -sf /usr/local/bin/brain-pdf /usr/local/bin/2nd-brain-parser
> docker commit --change 'USER user' --change 'WORKDIR /home/user/work' \
>               --change 'CMD ["sleep","infinity"]' \
>               rn-parser ghcr.io/benkorea/2nd-brain-parser:latest && docker rm rn-parser
> docker tag ghcr.io/benkorea/2nd-brain-parser:latest ghcr.io/benkorea/2nd-brain-parser:$(date +%Y.%m.%d)
> echo "$GHCR_PAT" | docker login ghcr.io -u <user> --password-stdin
> docker push ghcr.io/benkorea/2nd-brain-parser --all-tags
> ```
> 향후 망이 좋을 때 빌드소스를 `2nd-brain-parser` 로 정식 rename·재빌드하면 심링크 브리지는 불필요.
