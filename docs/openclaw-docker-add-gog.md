...
`~/.openclaw` 는 컨테이너의 `/home/node/.openclaw` 로 마운트되므로, 그 아래 `bin/` 에 두면 컨테이너에서 보입니다.
```bash
mkdir -p ~/.openclaw/bin
cp "$(command -v gog)" ~/.openclaw/bin/gog && chmod +x ~/.openclaw/bin/gog
```
**✅ 검증** (정적 바이너리 + 호스트에 존재):
```bash
file ~/.openclaw/bin/gog   # "statically linked" 포함해야 함
```
...
봇 워크스페이스(`~/.openclaw/workspace/USER.md`)에 **gog 인증 계정**을 명시해, 봇이 올바른 계정으로 호출하게 합니다:
